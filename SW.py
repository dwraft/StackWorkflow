...
class Stack():
    ...
    def make_search_url(self): #Taking a search term as input, turns search_url into a viable url
        keyword=self.search_term.replace(" ","%20")
...
class execute: #The program that when called will execute and run the StackWorkflow code
    def __init__(self, search_term):
        self.search_term=search_term
    def run(self):
        x=Stack(self.search_term)
        search_summary=x.condense_questions(x.make_search_url())
        count=1
        ...
        while True:
            ...
            answer=x.get_answer(x.make_answer_url(search_summary, index))
            print(answer)
